...
## --------------------------------------------------------------------------- #
##  ApPHP DataGrid version 4.2.8 (09.01.2014) - FOR ADMIN PANEL FREE ONLY      #
...
Class DataGrid
{
                            ...
                            foreach($splitted_fldTableFieldName as $separated_word){
                                ...
                                if($requested_filter_field_operator != ""){                                
                                    ...
                                    if(substr_count($requested_filter_field_operator, "like") > 0){
                                        $separated_word = str_replace('\"', $this->RealEscapeString('\\\"'), $separated_word); // double quotation mark 
                                        $separated_word = str_replace("\'", $this->RealEscapeString("\\\'"), $separated_word); // single quotation mark
                                    }else{
                                        $separated_word = str_replace('\"', $this->RealEscapeString('\\"'), $separated_word); // double quotation mark 
                                        $separated_word = str_replace("\'", $this->RealEscapeString("\\'"), $separated_word); // single quotation mark                                        
                                    }
    ...
    protected function DrawControlPanel(){       
        ...
        if($this->filtering_allowed || $this->exporting_allowed || $this->printing_allowed){
            ...
            if($this->filtering_allowed && ($this->mode == "view") && ($req_mode != "update") && ($req_mode != "delete")){
                if($req_print != true){                
                    echo "<td align='right' style='width: 20px;'><a style='cursor:pointer;' onClick='window.location.href=self.location;' class='".$this->css_class."_class_a'><img src='".$this->directory."images/".$this->css_class."/recycle.gif' alt='".$this->lang['refresh_page']."' title='".$this->lang['refresh_page']."'></a>";
                }
            }        
            echo "</tr>";
            echo "</table>";
        ...
        if($this->filtering_allowed){
            ...
            if($req_print != true){
                if($req_onSUBMIT_FILTER != ""){
                    ...
                    echo "<input class='".$this->css_class."_class_button' type='button' value='".$this->lang['reset']."' onClick='window.location.href=\"".$this->HTTP_URL.$curr_url."\"'>&nbsp;";
                }
    ...
    protected function DrawTabular(){
        ...
        if(isset($this->modes['add'][$this->mode]) && $this->modes['add'][$this->mode] &&
           isset($this->modes['add']['show_add_button']) && ($this->modes['add']['show_add_button'] == "outside")){
            ...
            echo "<td align='".(($this->direction == "ltr") ? "left" : "right")."'><b>";
                ...
                $this->DrawModeButton("add", $curr_url, $this->lang['add_new'], $this->lang['add_new_record'], "add.gif", "\"javascript:window.location.href='".$this->HTTP_URL.$curr_url."';\"", false, "", "");                        
            echo "</b></td>";
            echo "</tr>";
            echo "</table>";
        ...
        for($r = $this->row_lower; (($r >=0 && $this->row_upper >=0) && ($r < $this->row_upper) && ($r < ($this->row_lower + $this->req_page_size))); $r++){            
            ...
            if(($r == $this->row_lower) && ($this->rid == -1) && ($req_mode == "add")){
                ...
                $this->DrawModeButton("cancel", $cancel_url, $this->lang['cancel'], $this->lang['cancel'], "cancel.gif", "\"return ".$this->uniquePrefix."verifyCancel('".$cancel_url."'); javascript:window.location.href='".$this->HTTP_URL.$cancel_url."'\"", false, $this->nbsp, "");
                $this->ColClose();                
            ...
            if(isset($this->modes['edit'][$this->mode]) && $this->modes['edit'][$this->mode]){
                if(($this->mode == "edit") && (intval($this->rid) == intval($row[$this->GetFieldOffset($this->primary_key)]))){
                    ...
                    if(isset($_REQUEST[$this->uniquePrefix.'mode']) && $_REQUEST[$this->uniquePrefix.'mode'] === "add") {
                        ...
                        $this->DrawModeButton("cancel", $cancel_url, $this->lang['cancel'], $this->lang['cancel'], "cancel.gif", "\"javascript:window.location.href='".$this->HTTP_URL.$cancel_url."'\"", false, $this->nbsp, "");
                    }                    
                    $this->ColClose();
                }else {                                                            
                        ...
                        if (isset($this->modes['edit']['byFieldValue']) && ($this->modes['edit']['byFieldValue'] != "")){
                            if($this->GetFieldOffset($this->modes['edit']['byFieldValue']) == "-1"){
                                if($this->debug){
                                    ...
                                    $this->ColOpen("center",0,"nowrap",$main_td_color, $this->css_class."_class_td_main");                                    
                                    $this->DrawModeButton("edit", $curr_url, $this->lang['edit'], $this->lang['edit_record'], "edit.gif", "\"javascript:window.location.href='".$this->HTTP_URL.$curr_url."'\"", false, $this->nbsp, "");
                                }
                            }else{
                                $this->ColOpen(($this->direction == "rtl")?"right":"left",0,"nowrap",$main_td_color, $this->css_class."_class_td_main");
            ...
            if(isset($this->modes['details'][$this->mode]) && $this->modes['details'][$this->mode]){
                ...
                $this->SetUrlString($curr_url, "filtering", "sorting", "paging");                
                $this->ColOpen("center",0,"nowrap");
                $this->DrawModeButton("details", $curr_url, $this->lang['details'], $this->lang['view_details'], "details.gif", "\"javascript:window.location.href='".$this->HTTP_URL.$curr_url."';\"", false, $this->nbsp, "");                        
                $this->ColClose();
            }
            if(isset($this->modes['delete'][$this->mode]) && $this->modes['delete'][$this->mode]){
    ...
    protected function DrawColumnar(){
        ...
        if(isset($this->modes['add'][$this->mode]) && $this->modes['add'][$this->mode]){
            ...
            $this->RowOpen($r, $this->rowColor[0]);            
                ...
                $this->DrawModeButton("add", $curr_url, $this->lang['add_new'], $this->lang['add_new'], "add.gif", "\"javascript:window.location.href='".$this->HTTP_URL.$curr_url."';\"", true, "", "");                        
                $this->MainColClose();
            $this->RowClose();
            $this->TblClose();                
        }
        ...
        if(($r == $this->row_lower) && ($this->rid != -1)){
            ...
            if($this->mode === "details"){
                ...
                if($req_print != ""){
                    ...
                    echo ";'>";
                    echo $this->DrawModeButton("cancel", $cancel_url, $this->lang['back'], $this->lang['back'], "cancel.gif", "\"javascript:window.location.href='".$this->HTTP_URL.$cancel_url."'\"", false, "", "");
                    echo "</div>";
                }
            }else{
                // if not new row
            ...
            function ".$this->uniquePrefix."verifySelected(param, button_type, flag_name, flag_value){
                if(confirm('Are you sure you want to carry out this operation?')){
                    ...
                    if(found){
                        ...
                        window.location.href = document_location_href;
                    }else{
    ...
    protected function PagingSecondPart($lu_paging=false, $upper_br, $lower_br, $type="1"){
        ...
        if($this->pages_total >= 1){
            ...
            $text .= "\n<script type='text/javascript'>\n<!--\n";
            $text .= "function ".$this->uniquePrefix."setPageSize".$type."(){window.location.href = '$href_string&".$this->uniquePrefix."page_size='+document.frmPaging$this->uniquePrefix".$type.".page_size".$type.".value;}";
            $text .= "\n//-->\n</script>\n";
    ...
    protected function GetFieldValueByType($field_value, $ind, $row, $field_name="", $m_field_req_type=""){
        // Un-quote string quoted by $this->RealEscapeString()
        ...
        if($this->mode === "view"){
            if(array_key_exists($field_name, $this->columns_view_mode)){
                ...
                $field_value = htmlentities($this->RemoveBadChars(strip_tags($field_value), 'sanitize'));                                    
        ...
        // -= ADD / EDIT / DETAILS MODE =-    
            ...
            if(array_key_exists($field_name, $this->columns_edit_mode)){
                ...
                $on_js_event = htmlentities($this->RemoveBadChars(strip_tags($field_property_on_js_event), 'sanitize'));                    
                            ...
                            switch($field_property_view_type){
                                case "radiobutton":
                                    if(is_array($this->columns_edit_mode[$field_name]["source"])){  // don't remove columns_edit_mode
										$select_array = $this->columns_edit_mode[$field_name]["source"];
                                    }else{
										$select_array = $this->GetEnumValues($this->tbl_name, $field_name);
                                    }                                        
									$ret_enum .= $this->nbsp.$this->DrawRadioButtons($this->GetFieldRequiredType($field_name).$field_name, $field_name, $select_array, $field_value, 'source', "", $disabled, $on_js_event, $field_property_radiobuttons_alignment).$this->nbsp;
                                    break;                            
                                case "dropdownlist":
                                default:                                
                                    if(is_array($this->columns_edit_mode[$field_name]["source"])){  // don't remove columns_edit_mode
    ...
    protected function SetCommonJavaScript(){
        ...
        if(($req_mode == "add") || ($req_mode == "edit")){
            ...
            if(isset($this->modes['cancel'][$this->mode]) && $this->modes['cancel'][$this->mode]){
                echo "\n<script type='text/javascript'>\n<!--";                                
                echo "\n function ".$this->uniquePrefix."verifyCancel(param){if(confirm(\"".$this->lang['cancel_creating_new_record']."\")){window.location.href=param;} else { return false;}};";
                echo "//-->\n</script>\n";                                           
            }
        }else{ // view mode
    ...
    protected function SetCommonJavaScriptEnd(){
        ...
        // set verify JS functions
        if(isset($this->modes['delete'][$this->mode]) && $this->modes['delete'][$this->mode]){
            echo "\nfunction ".$this->uniquePrefix."verifyDelete(param){if(confirm(\"".$this->lang['delete_this_record']."\")){window.location.href=param;} else { window.event.cancelBubble = true; return false;}};";
        }        
        echo "\n//-->\n";
        echo "</script>";        
    }
    ...
    protected function ExportDownloadFile($file_name){
        ...
        " window.location.href = '".$this->directory."scripts/download.php?dir=".$this->exporting_directory."&file=".$file_name."'; ".
        "} else {".
        " window.close();".
        "}".
        "\n//-->\n</script>";
    ...
    protected function IsPasswordCrypted($field_name, $fldValue){
        ...
        $field_property_cryptography_type = $this->GetFieldProperty($field_name, "cryptography_type", "edit", 'lower');
    ...
    protected function PreparePasswordDecryption(){
        ...
        foreach($this->columns_edit_mode as $column_name => $column_array){
            ...
            if($field_property_type == "password"){
                ...
                $field_property_cryptography_type = $this->GetFieldProperty($column_name, "cryptography_type", "edit", 'lower');
